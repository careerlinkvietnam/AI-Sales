...
#### ルール4: エラー発生時は即座に確認・修正
- **エラーが出たら次に進まない**
- エラーの原因を調査し、修正してから再開する
- 原因不明の場合はユーザーに確認する
...
#### ルール17-B: 下書き作成済み企業の送信確認と更新手順

「下書き作成済み」でCRM Action IDがある企業について、定期的にメール送信を確認し、送信済みであれば更新する。

**確認手順:**
```bash
npx tsx scripts/pre_check.ts <企業ID> <対象月>
```

**判断基準:**

| Gmail送信日時 | 現在のタグ | アクション |
|--------------|-----------|-----------|
| 今回の下書き作成日より後 | 対象月（例：1月） | CRM更新 + タグ更新 |
| 今回の下書き作成日より後 | 対象月以外（例：4月） | CRM更新のみ、タグ更新しない |
| 古いメール（下書き作成前） | - | 何もしない（今回の送信ではない） |
| 未送信 | - | 何もしない（下書きのまま） |

**★重要★ 送信日時の確認:**
- pre_check.tsで表示される送信日時を必ず確認する
- 下書き作成日（SESSION_HANDOFF.md更新日）より**後**の日時であること
- 例：2025年10月のメール ≠ 2026年1月作成の下書きの送信

**更新手順（送信確認後）:**
```
□ 1. 現在のタグを確認（pre_check.tsのチェック1）
□ 2. タグが対象月の場合のみ以下を実行：
     a. CRM Actionを「メール送付済み」に更新
     b. タグを+3ヶ月に更新
     c. SESSION_HANDOFF.mdを更新
□ 3. タグが対象月以外の場合：
     a. CRM Actionを「メール送付済み」に更新のみ
     b. タグは更新しない
```
...
| 16759 | Yoshimoto Mushroom Vietnam | minh.nguyen@ymush.com.vn | ✅ メール送信済み | CRM Action 234835、タグ4月に更新済み |
| 16734 | Dover VN | son@doverseafoods.com | ✅ メール送信済み | CRM Action 234836、タグ4月に更新済み |
| 16725 | MMK Vietnam | thaolinh-nguyen@mmknet.com | ✅ メール送信済み | CRM Action 234868、タグ4月に更新済み |
| 16658 | Blended Asia | t.tatsumi@blended-asia.com | ✅ メール送信済み | CRM Action 234838、タグ4月に更新済み |
...
| 15353 | SHOCHIKU | satoru_kamiyama@shochiku.co.jp | ✅ メール送信済み | CRM Action 234845、タグ4月に更新済み |
...
### 1月連絡の処理状況（2026-01-30時点）

| 状態 | 件数 | 説明 |
|------|------|------|
| ✅ メール送信済み | 26社 | CRM更新＋タグ4月に更新完了 |
| 📝 下書き作成済み（未送信） | 20社 | ユーザーの送信待ち |
| ⏭️ スキップ | 6社 | 3ヶ月以内連絡済み |
| ❌ メールなし | 27社 | 連絡先不明 |
...
1. **下書き20社の送信確認**（ルール17-Bに従う）
   - pre_check.tsで送信確認
   - 送信日が下書き作成日（2026/01/30）より後か確認
   - 送信済み＆1月タグ → CRM更新＋タグ更新
   - 送信済み＆1月以外のタグ → CRM更新のみ
